...
def get_args_parser():
    ...
    parser.add_argument('--resume', default='TransTrack/crowdhuman_final.pth', help='resume from checkpoint')
    ...
    # demo
    parser.add_argument('--video_input', default='TransTrack/test.mp4')
...
def resize(image, size=500, max_size=1333):
...
def main(args):
    ...
    while res:
        ...
        x = int(cv2.waitKey(30)) - 48
        if x not in dontshow:
            dontshow.append(x)
        print(dontshow)
        for ret in res_track:
            ...
            if tracking_id not in dontshow:
        ...
        if key - 48 in dontshow:
            dontshow.remove(key-48)
